...
### 今日完成
- 基于 JDBC 实现数据库版学生管理系统
- 将 Day1 控制台菜单和 Day5 JDBC 数据库操作结合
- 新增 JdbcStudentManager 控制台菜单入口
- 使用 StudentDao 完成学生表的增删改查
- 实现查询所有学生、根据学号查询、新增、修改、删除功能
- 解决 JDBC 运行时找不到 MySQL 驱动的问题
- 在 JdbcUtil 中手动加载 MySQL 驱动类
...
### 今日完成
- 将数据库连接信息从 Java 代码中抽离出来
- 在 resources 目录下新增 db.properties 配置文件
- 修改 JdbcUtil，通过 Properties 读取数据库配置
- 使用 ClassLoader 读取 resources 下的配置文件
- 将 StudentDao 中的数据库操作改为 try-with-resources
- 优化 findAll、findById、add、update、delete 方法
- 减少手动关闭 Connection、PreparedStatement、ResultSet 的代码

### 今日技术点
- properties 配置文件
- Properties 类读取配置
- ClassLoader 加载 resources 文件
- try-with-resources 自动关闭资源
- JDBC 代码优化
- DAO 层代码规范化

### 今日总结
Day7 主要是对 day05-jdbc-demo 进行代码优化。
